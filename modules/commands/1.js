module.exports.config = {
  ...
  admins: ['61574596121645', 'admin_id_2'] // Thay đổi admin_id_1, admin_id_2 bằng ID thực tế của admin
};
  ...
  if (mentions && Object.keys(mentions).length > 0) {
    for (const [ID, name] of Object.entries(mentions)) {
      if (ID in busyData) {
        ...
        api.sendMessage(`${mentioner},\ntag cái gì?☝🏿 ${infoBusy.lido ? ` đang ${infoBusy.lido}` : "."}`, threadID, async (error, messageInfo) => {
          if (!error) {
